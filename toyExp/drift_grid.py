...
def compute_plan_sinkhorn(x, y, eps, n_iter=100):
    dist = pairwise_l2(x, y)
    log_T = -dist / eps
...
def drift_particles(x_init, y_target, plan_fn, eps, eta, n_steps, **kwargs):
    N = x_init.shape[0]
...
for mi, (mname, pfn, kw) in enumerate(plan_fns):
    for ei, eps in enumerate(eps_values):
        ...
        traj = drift_particles(x_source, y_target, pfn, eps, eta, n_steps, **kw)
        results[(mi, ei)] = traj
...
fig.suptitle(
    r'Particle Drift Trajectories ($N$=%d, $\eta$=%.1f, steps=%d)'
    % (N, eta, n_steps),
    fontsize=20, fontweight='bold', y=1.01
)

plt.savefig('/home/hep3/drift/outputs/drift_grid_3x4.png', dpi=200,
            bbox_inches='tight', facecolor='white')
plt.savefig('/home/hep3/drift/outputs/drift_grid_3x4.pdf',
            bbox_inches='tight', facecolor='white')
print("Done — saved drift_grid_3x4.png and .pdf")
